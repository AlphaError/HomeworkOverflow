...
select quest.qid, quest.title, a.numA, a.numQ
from (Select c.qid, title, count(aid) as numA, numQ
	  from answers right join (
	  	  SELECT title, qid, count(qid) as numQ
		  FROM Questions JOIN Categories USING(qid), keywords
		  where LOCATE(keywords.word, questions.title) = 1
	  ...
	  group by c.qid
	  order by numQ desc, numA desc) as a, Questions as quest
where quest.qid = a.qid;
